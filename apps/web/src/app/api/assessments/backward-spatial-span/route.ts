...
// Real Assessment/AssessmentResult persistence for the Backward Spatial
// Span near-transfer assessment (docs/kanban.md's "Near-transfer
// assessment: alternate spatial-span variant" card) — the SPATIAL-domain
// twin of /api/assessments/backward-digit-span/route.ts, same shape:
// the client runs the whole fixed-staircase procedure itself
// (packages/cognitive-engine's BackwardSpatialSpanAssessment, no
// adaptive engine involved) and calls this route once, at the end.

const bodySchema = z.object({
  finalSpan: z.number().int().min(0).max(BACKWARD_SPATIAL_SPAN_MAX),
...
export async function POST(request: Request) {
  ...
  // (apps/web/src/app/assessments/backward-spatial-span/page.tsx) — a
  ...
  // docstring and BackwardSpatialSpanScore's own field comments for why.
  const confidenceInterval = calculateProportionConfidenceInterval(finalSpanCorrect, finalSpanTrials);

  let assessment = await prisma.assessment.findFirst({
    where: { type: "NEAR_TRANSFER", name: BACKWARD_SPATIAL_SPAN_ASSESSMENT_NAME },
  });
  if (!assessment) {
    assessment = await prisma.assessment.create({
      data: {
        name: BACKWARD_SPATIAL_SPAN_ASSESSMENT_NAME,
        type: "NEAR_TRANSFER",
        description:
          "A fixed ascending-span staircase measuring backward spatial recall on a 3x3 grid — a near-transfer measure for spatial working-memory training, distinct in task and form from the trained (forward-recall) Spatial Sequence exercise.",
      },
    });
  }
  ...
  const result = await prisma.assessmentResult.create({
    data: {
      ...
      scoreSummary: {
        ...
        minPossibleSpan: BACKWARD_SPATIAL_SPAN_MIN,
        maxPossibleSpan: BACKWARD_SPATIAL_SPAN_MAX,
      },
    },
  });
