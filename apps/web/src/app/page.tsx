...
// deliberately scoped down: the weekly activity bar, XP/Training Level
// indicator, upcoming-assessment/achievement cards, and the Progress/
// Science/Profile bottom nav are all real future cards of their own
...
// "Start Training" leads into /train/session, which strings those
// exercises together without returning here in between, and the streak
// indicator reads the real Streak row written by that same session's
// completion (see apps/web/src/lib/streak.ts, the Streaks card).
      ...
      {session?.user ? (
        <div className="flex w-full max-w-sm flex-col gap-4 text-left">
          {streak && streak.currentStreakDays > 0 ? (
            <div className="flex items-center gap-2 text-[13.5px] font-semibold text-text-2" data-testid="streak-indicator">
