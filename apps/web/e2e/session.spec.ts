...
// Verifies docs/kanban.md's Session orchestration card end to end: after
// onboarding, /train/session strings the day's exercises together
// without returning to a menu (Home -> Exercise -> transition ->
// Exercise -> ... -> Session Complete), and each exercise's real Trial
// rows + updated DifficultyState land in Postgres via
// POST /api/training-sessions/:id/exercises, with the TrainingSession
...
test("complete onboarding, run a full training session, and see real Trial rows land", async ({ page }) => {
  ...
  expect(trainingSession?.status).toBe("COMPLETED");
  expect(trainingSession?.totalDurationSeconds).toBeGreaterThan(0);

  const trials = await prisma.trial.findMany({ where: { trainingSessionId: trainingSession!.id } });
  expect(trials.length).toBeGreaterThan(0);

  const distinctTaskVersions = new Set(trials.map((t) => t.taskVersionId));
  expect(distinctTaskVersions.size).toBe(3); // one per exercise run this session
});
