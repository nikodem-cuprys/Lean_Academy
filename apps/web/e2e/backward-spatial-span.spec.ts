...
// Same rationale as e2e/backward-digit-span.spec.ts — this is the
// SPATIAL-domain twin. Verifies the real client-side interaction loop
// (study-phase timing, tap-to-recall, staircase scoring, results
// transition) in an actual Chromium browser, plus the real
// Assessment/AssessmentResult DB rows and the periodic "last taken"
// status text on a second visit. Requires a running production server
// with a real Postgres behind it; not run as part of `pnpm test`
// (vitest-only, unit tests).
//
// Reads the grid sequence as it's actually lit (via the
// "highlighted-cell" test id, the same technique already proven on
// e2e/spatial-sequence.spec.ts) and taps it back in reverse order every
// trial, so the bot always succeeds and the staircase runs all the way
// to the ceiling (grid size, 9) — a genuine correct-recall path through
// the real BackwardSpatialSpanAssessment engine. The failure-
// termination path (two consecutive misses) is covered by
// packages/cognitive-engine's own unit tests instead of here, matching
// how this project already splits "genuine browser interaction +
// persistence" (e2e) from "engine edge cases" (unit tests).

const email = `e2e-backward-spatial-span-${Date.now()}@example.com`;
...
test("sign up, run the full staircase with correct backward recall to the ceiling, and see the real DB result", async ({
  ...
  while ((await resultsScreen.count()) === 0) {
    ...
    for (const position of [...sequence].reverse()) {
  ...
  // staircase must have run all the way to the max span (grid size, 9).
  ...
  const assessment = await prisma.assessment.findFirstOrThrow({
    where: { type: "NEAR_TRANSFER", name: "Backward Spatial Span" },
  });
