...
`apps/web` ships in six languages — English, Polish, Spanish, German, French, Simplified Chinese — via `next-intl` **without locale-prefixed URLs** (every route stays where it is). The locale config lives in `apps/web/src/i18n/`; the messages live in `apps/web/src/messages/<locale>.json`, and `en.json` is the source every other file is translated from.

- **Every new user-facing string goes into all six message files in the same change.** `pnpm typecheck` enforces this: `t("...")` keys are typed against `en.json` (`src/i18n/global.d.ts`), and `src/i18n/messages-parity.ts` fails the build if any locale file is missing a key or has an extra one. Placeholders (`{count}`, ICU plurals) and `<strong>` rich-text tags must match `en.json` too. The typecheck doesn't catch mismatches there, so check them by hand. Polish needs `one/few/many/other` plural branches.
- **Locale resolution** (`src/i18n/locale.ts`): `NEXT_LOCALE` cookie (the switcher's choice on this device) → the signed-in user's `User.locale` (carried in the session JWT) → `Accept-Language` → English. Traditional-Chinese tags (`zh-TW`/`zh-HK`/`zh-Hant`) deliberately don't match `zh-CN`. `LanguageSwitcher.tsx` posts to `/api/locale`, which sets the cookie and, when signed in, `User.locale`. Signup and new OAuth users get the language they signed up in. Transactional emails go out in the recipient's `User.locale`, not the requester's.
- **Gotcha: anything that changes who's signed in must do a full page load, not `router.push`.** A client-side navigation keeps the root layout's `<html lang>` and `NextIntlClientProvider` messages from the previous page, so client components would stay in the old language. `AuthForm.tsx` uses `window.location.assign("/")` for this reason (caught by `e2e/i18n.spec.ts`).
- **Deliberately English-only in every language:** reading passages and their comprehension questions, because the WPM ladder and the 70% comprehension floor are calibrated on English text and WPM isn't comparable across languages. The same goes for `data/evidence-registry.json` content on `/science` (module names, target constructs), which is reviewed scientific text. Both are marked `lang="en"`, and a non-English UI shows a one-line note saying so. Don't machine-translate either without an evidence-review pass (`docs/evidence-review.md` §8).
- DB-stored English copy (Achievement titles, Challenge titles/descriptions) is translated at render time by its stable `key`/`slug`. The DB text is only a fallback. Server-built English summary strings that e2e specs assert on (`progressLabel`, `personalBestLabel`) are kept as-is, and the UI renders from the structured fields alongside them instead.
- The non-negotiable content rules below apply to every translation, not just English.
...
```text
apps/
  web/     Next.js 16 (App Router, Turbopack). See docs/kanban.md's Done section and docs/development-plan.md for current phase status. apps/web/e2e/accessibility.spec.ts (axe-core, plus a manual keyboard-nav check) runs against every real screen — keep it green when adding new interactive elements, and prefer real <button>/<input> elements with proper roles over div-onClick, which axe won't catch on its own but a keyboard-only user will hit immediately. Full auth flow built: /login, /signup, /verify-email, /forgot-password, /reset-password. /train/n-back, /train/complex-span, /train/dice-sum, /train/spatial-sequence, and /train/reading are the five real exercises, each usable standalone or embedded in a session via the optional SessionModeProps (initialDifficulty/onComplete — see apps/web/src/lib/session-types.ts). /train/dice-sum is wired into TrainingSessionRunner's method map but deliberately not yet part of onboarding calibration or the daily getTodaysTraining rotation — reachable standalone and via the home page's quick links only, a separate scope decision from actually competing with N-Back/Complex Span for the daily WORKING_MEMORY slot. /onboarding (goals -> time -> experience -> calibration -> recommended level) writes the first TrainingPlan/DailyGoal/DifficultyState/AssessmentResult rows; it's an opt-in link on the home page, not a mandatory post-signup redirect yet. The real Home screen (apps/web/src/app/page.tsx) shows a "Today's Training" card from apps/web/src/lib/todays-training.ts and strings that day's exercises together via /train/session (<TrainingSessionRunner>, apps/web/src/components/TrainingSessionRunner.tsx) without returning to a menu between them, persisting real Trial rows and advancing DifficultyState per exercise through /api/training-sessions/*. /progress (<ProgressView>, apps/web/src/lib/progress-data.ts) reads those same Trial/DifficultyState rows directly — a user who has only tried exercises via their standalone /train/<exercise> routes (which never persist) shows up honestly as having nothing trained yet. /progress/trends (<LongitudinalTrendsView>, apps/web/src/lib/trend-data.ts, premium-gated) shows a real multi-point time series per trained task/reading metric instead of a before/after snapshot, gated behind a fixed 5-session/14-real-day-span floor per chart so a handful of same-day sessions never draws a misleading trend line. /advanced (<AdvancedHubView>/<AdvancedLessonView>, apps/web/src/lib/advanced-settings.ts + advanced-runs.ts) is the Advanced tab: fully custom per-exercise parameters (timings, grid sizes, fixed reading WPM, run length, adaptive on/off), run in the exercises' session mode but saved only to the AdvancedRun table, never TrainingSession/Trial/DifficultyState/XP, so advanced lessons stay separate from main and daily training. Keep it that way. /science (<ScienceView>, apps/web/src/lib/science-data.ts) renders directly from data/evidence-registry.json, cross-referenced against real TaskDefinition rows for which exercises are actually implemented. Playwright E2E tests live in apps/web/e2e/ (vitest is unit-only — see Commands).
...
packages/
  ...
  cognitive-engine/    NBackTask, ComplexSpanTask, SpatialSequenceTask, and DiceSumTask done (all wrap adaptive-engine; difficulty *is* N / set size / sequence length / dice count respectively). Verbal sequencing, method of loci still placeholders — follow this pattern: task-specific stimulus/scoring logic here, difficulty adaptation delegated to adaptive-engine, never reimplemented.
