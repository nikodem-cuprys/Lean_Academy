...
- **[Working Memory] N-Back exercise screen (apps/web)** — `/train/n-back`, auth-gated, `<NBackExercise>` running a real 20-trial session against `NBackTask` with `performance.now()`-based stimulus/response timing, a `visibilitychange` listener that excludes an interrupted trial from scoring entirely (not just flags it), and a results screen with honest per-outcome copy (level-up/level-down/held-steady framed as adaptive-engine behavior, never as an ability claim). Two deliberate departures from `prototype/Exercise.dc.html`, both explained in the component's own header comment: the "Exercise 1 of 3" dots became a real "Trial N of 20" progress bar (no multi-exercise session exists yet to make dots honest), and the domain tag was corrected from the prototype's "SPATIAL SEQUENCE" label to "N-BACK" (matching what the task actually is). **Verification is partial and split by layer, not glossed over**: build/typecheck/lint all pass, and server-rendered HTML was confirmed correct over HTTP (redirects to `/login` when signed out, renders the right domain tag/instructions/initial trial count when signed in). The actual client-side interaction loop — stimulus timing, tap-to-respond, the results transition — has **not** been visually exercised in a running browser, because the Claude-in-Chrome extension wasn't connected in this environment. Do the manual click-through before treating the timing/interaction behavior as proven; the engine logic itself is separately unit-tested (see above), which somewhat limits the blast radius of anything the UI layer might still get wrong. Owner: Senior Full-Stack Engineer / Senior UI/UX Designer.
...
- **[Working Memory] Manually verify the N-Back exercise in a real browser** — the interactive loop (stimulus timing, tap-to-respond, results transition) has only been verified server-side over HTTP, not click-tested live; the Claude-in-Chrome extension wasn't connected when it was built. Do a real click-through at /train/n-back before treating this as fully proven.
