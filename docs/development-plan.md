...
**Size:** L.
**Status:** Underway — see `docs/kanban.md`'s Phase 6 breakdown for six concrete cards. Five are done: a real psychometric scoring library (`packages/psychometrics`'s real `calculateDPrime`/`calculateSignalDetectionRates`/`calculateProportionConfidenceInterval`); both genuine near-transfer assessments — Backward Digit Span for Working Memory and Backward Spatial Span for Spatial, each a fixed ascending-span staircase (not the adaptive engine), with real `Assessment`/`AssessmentResult` rows carrying a real confidence interval and a shared 14-day periodic-trigger rule (`apps/web/src/lib/near-transfer-assessment.ts`); wiring the Progress page's "Similar Cognitive Tasks" tab to that real data, replacing its placeholder with each assessment's actual score, confidence interval, and a real "Take"/"Retake" state per assessment; and an expanded/rotating reading-passage bank — `packages/reading-engine`'s passage pool grew from 5 same-tier passages to 9 across three genuinely different difficulty tiers, and passage selection now prefers each user's real least-recently-seen passage (computed from real `Trial` history) instead of the previous random pick, a real alternate-forms fix for the pure-practice-effect problem — see `docs/kanban.md`'s Done section for all five. Remaining in Backlog: a longitudinal trend view, genuinely not actionable yet since it needs weeks of real elapsed usage time first. No far-transfer assessment card exists — `docs/evidence-review.md` doesn't support a far-transfer claim for any current module, and the Progress page's "Broader Transfer" tab already handles that honestly as a static disclaimer needing no new data.
