...
**Size:** L.
**Status:** Underway — see `docs/kanban.md`'s Phase 6 breakdown for six concrete cards. The first, a real psychometric scoring library (`packages/psychometrics`'s real `calculateDPrime`/`calculateSignalDetectionRates`/`calculateProportionConfidenceInterval`, foundational for the rest), is done — see `docs/kanban.md`'s Done section. Remaining: two genuine near-transfer assessments (Backward Digit Span for Working Memory, an alternate spatial-span variant for Spatial, both now unblocked), an expanded/rotating reading-passage bank (alternate forms), wiring the Progress page's "Similar Cognitive Tasks" tab to the real assessment data once it exists, and a longitudinal trend view (sequenced last since it needs the most real elapsed usage time). No far-transfer assessment card exists — `docs/evidence-review.md` doesn't support a far-transfer claim for any current module, and the Progress page's "Broader Transfer" tab already handles that honestly as a static disclaimer needing no new data.
