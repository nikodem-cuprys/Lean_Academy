...
**Size:** L.
**Status:** Underway — see `docs/kanban.md`'s Phase 5 breakdown for the five concrete cards (Streaks, Achievements, XP + Training Level, Personal Bests, Weekly Challenges) with real acceptance criteria, dependencies, and sequencing. Streaks and Achievements are both done: a real `Streak` row updates on every completed `TrainingSession` with a genuine freeze-based protection mechanic, and all 11 real achievements from `prototype/Achievements.dc.html` are seeded and awarded from real completion events (session counts, streak length, task difficulty history, reading comprehension, first assessment) — both verified end-to-end in `apps/web/e2e/streaks.spec.ts` and `apps/web/e2e/achievements.spec.ts`, with a real `/achievements` page. The remaining three (XP + Training Level, Personal Bests, Weekly Challenges) are sequenced next for reasons detailed in the kanban (new schema needed for XP/Level; Personal Bests and Weekly Challenges each build on a placeholder the Achievements card left behind — its own lightweight personal-best check and fixed weekly-session target). Leagues/social features are explicitly out of scope for this phase per `project_prompt.txt`'s "optional later-stage feature" framing.
