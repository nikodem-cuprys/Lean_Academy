...
**Size:** L.
**Status:** Underway — see `docs/kanban.md`'s Phase 6 breakdown for six concrete cards. Two are done: a real psychometric scoring library (`packages/psychometrics`'s real `calculateDPrime`/`calculateSignalDetectionRates`/`calculateProportionConfidenceInterval`, foundational for the rest) and the first genuine near-transfer assessment, Backward Digit Span for Working Memory (a fixed ascending-span staircase, real `Assessment`/`AssessmentResult` rows with a real confidence interval, a shared 14-day periodic-trigger rule, and a new `/assessments/backward-digit-span` screen linked from Progress) — see `docs/kanban.md`'s Done section for both. In Ready: the alternate spatial-span variant for Spatial, built to reuse Backward Digit Span's same periodic-trigger helper. Remaining in Backlog: an expanded/rotating reading-passage bank (alternate forms), wiring the Progress page's "Similar Cognitive Tasks" tab to the real assessment data (needs both near-transfer assessments first), and a longitudinal trend view (sequenced last since it needs the most real elapsed usage time). No far-transfer assessment card exists — `docs/evidence-review.md` doesn't support a far-transfer claim for any current module, and the Progress page's "Broader Transfer" tab already handles that honestly as a static disclaimer needing no new data.
