...
**Size:** L.
**Status:** Underway — see `docs/kanban.md`'s Phase 6 breakdown for six concrete cards. Three are done: a real psychometric scoring library (`packages/psychometrics`'s real `calculateDPrime`/`calculateSignalDetectionRates`/`calculateProportionConfidenceInterval`, foundational for the rest) and both genuine near-transfer assessments — Backward Digit Span for Working Memory and Backward Spatial Span for Spatial, each a fixed ascending-span staircase (not the adaptive engine), with real `Assessment`/`AssessmentResult` rows carrying a real confidence interval, a shared 14-day periodic-trigger rule (`apps/web/src/lib/near-transfer-assessment.ts`), and their own `/assessments/*` screens linked from Progress's "Similar tasks" tab — see `docs/kanban.md`'s Done section for all three. Remaining in Backlog, both now unblocked: wiring the Progress page's "Similar Cognitive Tasks" tab to the real assessment data (needed both near-transfer assessments first — now satisfied), an expanded/rotating reading-passage bank (alternate forms), and a longitudinal trend view (sequenced last since it needs the most real elapsed usage time). No far-transfer assessment card exists — `docs/evidence-review.md` doesn't support a far-transfer claim for any current module, and the Progress page's "Broader Transfer" tab already handles that honestly as a static disclaimer needing no new data.
