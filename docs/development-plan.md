...
**Size:** L.
**Status:** Underway — see `docs/kanban.md`'s Phase 5 breakdown for the five concrete cards (Streaks, Achievements, XP + Training Level, Personal Bests, Weekly Challenges) with real acceptance criteria, dependencies, and sequencing. Streaks, Achievements, XP + Training Level, and Personal Bests are all done: a real `Streak` row updates on every completed `TrainingSession` with a genuine freeze-based protection mechanic; all 11 real achievements from `prototype/Achievements.dc.html` are seeded and awarded from real completion events; a real `XpEntry` ledger awards XP for completing training, consistency, trying new domains, and genuine milestones, with a real 150-XP/day anti-grind cap and a documented Training Level formula shown on Home; and a real per-task Personal Best view (computed at query time, no new table) shows on the Progress page and Session Complete, using the real Reading Efficiency Score for reading rather than a raw-pace comparison — verified end-to-end in `apps/web/e2e/streaks.spec.ts`, `apps/web/e2e/achievements.spec.ts`, `apps/web/e2e/xp.spec.ts`, and `apps/web/e2e/personal-bests.spec.ts`. Only Weekly Challenges remains, building on the fixed weekly-session-target placeholder the Achievements card left behind. Leagues/social features are explicitly out of scope for this phase per `project_prompt.txt`'s "optional later-stage feature" framing.
