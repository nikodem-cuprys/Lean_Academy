...
**Size:** L.
**Status:** Underway — see `docs/kanban.md`'s Phase 5 breakdown for the five concrete cards (Streaks, Achievements, XP + Training Level, Personal Bests, Weekly Challenges) with real acceptance criteria, dependencies, and sequencing. Streaks, Achievements, and XP + Training Level are all done: a real `Streak` row updates on every completed `TrainingSession` with a genuine freeze-based protection mechanic; all 11 real achievements from `prototype/Achievements.dc.html` are seeded and awarded from real completion events; and a real `XpEntry` ledger now awards XP for completing training, consistency, trying new domains, and genuine milestones, with a real 150-XP/day anti-grind cap and a documented Training Level formula shown on Home — verified end-to-end in `apps/web/e2e/streaks.spec.ts`, `apps/web/e2e/achievements.spec.ts`, and `apps/web/e2e/xp.spec.ts`. The remaining two (Personal Bests, Weekly Challenges) are sequenced next: Personal Bests builds on the lightweight personal-best check the Achievements card already left behind, and Weekly Challenges builds on the fixed weekly-session-target placeholder the Achievements card also left behind. Leagues/social features are explicitly out of scope for this phase per `project_prompt.txt`'s "optional later-stage feature" framing.
